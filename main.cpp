...
public:
  ...
  Main() : CPPPlugin() {
	  ...
	  _7fang.reserve(20);
	  _7fang.emplace_back(IDMAX, 1302349577, "汐铃星", 2000, 5000, 233333,50,150,100,100);
	  _7fang.emplace_back(IDMAX, 2416314577, "焈灵", 100, 110, 100);
  ...
  void onEnable() override {
    // 请在此处监听
		...
		Event::registerEvent<GroupMessageEvent>([](GroupMessageEvent hear) {
			...
			if (!hearPlainText.empty()) {
			  ...
			  if (hearString[0].length() > (unsigned)5) {
				  ...
				  if (hears == "焈灵") {
					  std::string shuchu = yuyan_chuli(hearString[0], "焈灵");
		...
		/*	break;*/
			...
			//		if (hears == "焈灵" || hears == "商店") {
			//			hear.group.sendMessage(PlainText("处于特殊秘境「有点脏的灰水晶球」，无法接触外界商店或联系外界小焈灵。"));
			//		}
			//		else {
			//			if (hears == "附身") {
			//				
...
/// </summary>
/// <param name="send">听到的，前提语（现在用"焈灵"）</param>
/// <returns>说出的</returns>
std::string yuyan_chuli(std::string& send, const std::string& bian) {

	using namespace std;

	std::string shuchu("叫焈灵干嘛呢！（凶@ ");
		...
		if (send.find("是") != string::npos) {
			...
			if (foudingshu_bu % 2 == 0) {
				...
				case(0): {
					...
					case(2):
						hou = send.erase(0, send.find("是"));
						shuchu = "焈灵不明白" + hou + "是什么，但焈灵觉得可以打你";
						break;
					default:
						shuchu = "error";
						break;
				...
				case(1): {
					...
					case(0): {
						...
						switch (rand() % 2 + 1) {
						case(1): shuchu = "为什么这样说呢，焈灵惹你生气了吗";
							break;
						case(2):
							hou = send.erase(0, send.find(bian));
							shuchu = "hhhhhhhh焈灵发现了" + hou + "!";
							xi++;
							break;
						}
					}
				...
				case(0): {
					...
					case(1): {
						...
						case(1):	shuchu = "呜。。。";
							break;
						case(2):	shuchu = "焈灵要求相应的时空规则证据与四维以上等价矩阵实例，不然你就是错的！";
							break;
						}
					}
						   break;
			...
			if (foudingshu_bu % 2 == 0) {
				...
				switch (hui_qian) {
				case(0):shuchu = "嗯！！（甩头）焈灵会努力学的！";
					break;
				case(1):shuchu = "要焈灵教你吗？焈灵可是天才哦";
					break;
	...
	//中间语部分；
		...
		if (send.find("抱") != string::npos || send.find("摸") != string::npos || send.find("媷") != string::npos || send.find("吃") != string::npos || send.find("敲") != string::npos || send.find("揉") != string::npos) {
			...
			case(1):	shuchu = "你真的要" + send.erase(0, send.find_first_of("摔刀扬死踢打看拖摇摆跑吵靠滑哭笑拿放扔捅踩屎滚爬")) + "吗.....";
				xi -= 1;
				break;
			case(2):	shuchu = ">、< 焈灵要求反击权";
				xi -= 2;
				break;
			case(3):	shuchu = "唔、为什么要这样对待焈灵.....";
				xi -= 5;
				break;
			case(4):	shuchu = "再欺负焈灵，焈灵要找主人告状了！q、q";
				xi -= 3;
				break;
			default:    shuchu = "error";
				break;
